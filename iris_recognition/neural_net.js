...
function Mind(opts) {
  ...
  this.hiddenUnits = opts.hiddenUnits || 10;
}
...
fs.readFile('data.txt','utf-8',(err,file_data) => {
  if (err) throw err;
  ...
  file_data = file_data.map((each) => {
    ...
    let num_d = each.splice(4,1);
    if (num_d[0] == 'Iris-setosa'){
      num_d = [1,0,0];
    }
    if (num_d[0] == 'Iris-versicolor'){
      num_d = [0,1,0];
    }
    if (num_d[0] == 'Iris-virginica'){
      num_d = [0,0,1];
    }
    each.push(num_d);
    return each;
  ...
  file_data.forEach((x,i) => {
    if (i == 47 || i == 48 || i == 49 || i == 50 || i == 98 || i == 99 || i == 100 || i == 148 || i == 149 || i == 150){
      test_cases.push(x);
    }
    else {
      data.push(x)
